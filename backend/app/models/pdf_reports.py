...
class AdminAnalyticsReportGenerator:
    ...
    async def _collect_donor_stats(self):
        ...
        for donor in donors:
            ...
            age = today.year - info.birth_date.year - ((today.month, today.day) < (info.birth_date.month, info.birth_date.day))
            ages.append(age)
        ...
        self.report_data['chronic_disease_percentage'] = (
            (chronic_disease_count / len(donors)) * 100 if donors else 0
        )
    ...
    async def _collect_registration_stats(self):
        ...
        self.report_data['registration_conversion'] = (
            (total_registrations - closed_count) / total_registrations * 100 
    ...
    def _create_user_stats_page(self, elements):
        ...
        self._create_pie_chart(
            elements,
            self.report_data.get('roles_distribution', {}),
    ...
    def _create_donor_stats_page(self, elements):
        ...
        self._create_pie_chart(
            elements,
            self.report_data.get('gender_distribution', {}),
        ...
        self._create_pie_chart(
            elements,
            self.report_data.get('donor_experience', {}),
            "Опыт донорства"
        )
    ...
    def _create_event_stats_page(self, elements):
        ...
        elements.append(Paragraph(f"Средняя заполняемость: {self.report_data.get('avg_occupancy', 0):.1f}%", styles["Body"]))
        elements.append(Paragraph(f"Среднее количество доноров: {self.report_data.get('avg_donors_per_event', 0):.1f}", styles["Body"]))
    ...
    def _create_line_chart(self, elements, data_dict, title, xlabel, ylabel):
        ...
        for h, v in zip(hours, values):
            plt.text(h, v + max(values)*0.05, str(v), 
    ...
    def _create_registration_dynamics_chart(self, elements):
        ...
        for i, count in enumerate(counts):
            plt.text(i, count + max(counts)*0.05, str(count), 
                     ha='center', va='bottom', fontsize=10, fontweight='bold')
